...
async function usaEndereco() {
  ...
  try {
    // 05 // Bloco try para tratamento de exceções
    // 06 // Utiliza await para aguardar a finalização da chamada assíncrona
    await end.setCep('72015565'); // 07 // Chama o método assíncrono que consome API ViaCEP
    ...
    console.log('UF:', end.getUf()); // 13
  } catch (erro) {
    // 14 // Captura qualquer erro que ocorra durante o await
