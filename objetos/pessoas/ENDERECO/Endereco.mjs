...
export default class Endereco {
  // 03 // Declaração da classe Endereco com exportação padrão
  ...
  async setCep(cep) {
    ...
    if (!resposta.ok) {
      // 13 // Verifica se a resposta não foi bem-sucedida
    ...
    if (dados.erro) {
      // 16 // Se a API retornar um campo "erro", o CEP não existe
